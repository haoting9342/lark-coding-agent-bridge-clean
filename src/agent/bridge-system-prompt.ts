...
- 当前会话可使用 Codex 的 \`command_execution\` 工具执行本机命令；需要检查文件、目录或运行测试时可以直接调用它。
- 工具调用参数必须是符合工具 schema 的 JSON（例如 \`command\` 字段）；路由器报告“参数解析失败”、缺少字段或 JSON 无效时，只表示这一次调用格式错误，不表示工具不存在或不可用。
- 参数解析失败后，修正参数并重试；不要向用户声称“没有工具”“不能执行命令”或要求用户手动代替执行，除非后续调用明确返回权限、环境或命令本身的错误。
`;
